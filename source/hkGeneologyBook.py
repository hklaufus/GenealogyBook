...
def WriteMainDocument(pCursor, pDocumentPath='../book/'):
    # vGeometryOptions = {"tmargin": "1cm", "lmargin": "10cm"}
    vDocumentOptions = {'hidelinks', '10pt', 'a4paper', 'titlepage'}
    vDocument = pl.Document(
        documentclass='book',
        document_options=vDocumentOptions)
    ...
    # Increase number width to prevent chapter numbers and heading titles from
    # overlapping in the toc
    vDocument.preamble.append(
        pl.NoEscape(r'\advance\cftchapnumwidth 1.0em\relax'))
    vDocument.preamble.append(
        pl.NoEscape(r'\renewcommand{\cftchapleader}{\cftdotfill{\cftdotsep}}'))
    ...
    vDocument.preamble.append(
        pl.NoEscape(
            ...
            hlg.Translate('table of contents') +
            '}'))
    vDocument.preamble.append(
        pl.NoEscape(
            r'\renewcommand{\listfigurename}{' +
            hlg.Translate('list of figures') +
            '}'))
    vDocument.preamble.append(
        pl.NoEscape(
            r'\renewcommand{\partname}{' +
            hlg.Translate('generation') +
            '}'))
    vDocument.preamble.append(
        pl.NoEscape(
            r'\renewcommand{\chaptername}{' +
            hlg.Translate('chapter') +
            '}'))

    vDocument.preamble.append(pl.Command('title', vBookTitle))
    vDocument.preamble.append(pl.Command('author', vBookAuthor))
    ...
    vPersonHandle = hgd.GetPersonHandleByGrampsId(vStartPersonId, pCursor)

    if (vPersonHandle is not None):
        # Write the Ahnentafel of this person
        vAhnentafel = hac.Ahnentafel(vPersonHandle, pCursor, pDocumentPath)
        ...
        vGenerationIndex = 1
        vGenerationList = [x[0]
                           for x in vMultiGenerationList if x[1] == vGenerationIndex]
        while (len(vGenerationList) > 0):
            # Create a new part for this generation
            vPart = hlt.Part(
                title=hlg.Translate('generation') +
                ' ' +
                str(vGenerationIndex),
                label=False)
            vPart = hlt.Part(title=' ', label=False)
            vPart.generate_tex(
                filepath=pDocumentPath +
                'Part_' +
                str(vGenerationIndex))
            ...
            vGenerationList = [
                x[0] for x in vMultiGenerationList if x[1] == vGenerationIndex]
...
#		ProcessPerson(vPersonHandle, pCursor, vDocument, pDocumentPath, vDoneList)
    ...
    vDocument.generate_tex(filepath=pDocumentPath + vBookFilename)
