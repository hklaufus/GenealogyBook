...
class Ahnentafel:
    ...
    def __init__(self, p_subject_handle: str, p_cursor, p_document_path: str='../book/', p_source_status: bool=False):
        ...
        self.__create_generation_list__(p_person_handle=self.__person_handle__, p_generation_index=1)

    def __add_person(self, p_person_handle: str, p_binary_tree_string: str):
        v_person = person.Person(p_person_handle, self.__cursor__)

        # Display progress
        logging.info("Adding: ", v_person.__given_names__, v_person.__surname__)
        ...
        logging.info(v_string)

        self.__chapter__.append(pl.NoEscape(v_string))

        v_father_handle: str = v_person.get_father_handle()
        if v_father_handle is not None:
            self.__add_person(p_person_handle=v_father_handle, p_binary_tree_string=p_binary_tree_string + 'M')
        else:
            logging.info("End of tree...")

        v_mother_handle = v_person.get_mother_handle()
        if v_mother_handle is not None:
            self.__add_person(p_person_handle=v_mother_handle, p_binary_tree_string=p_binary_tree_string + 'F')
        else:
            logging.info("End of tree...")
            ...
            for (v_person_handle, v_binary_tree_string) in p_generation_list:
                if v_person_handle is not None:
                    if not v_generation_started:
                        with self.__chapter__.create(pl.Section(numbering=False, title=language.translate('generation') + ' ' + str(v_generation_index), label=False)):
                    ...
                    v_person = person.Person(v_person_handle, self.__cursor__)
                    ...
                    if self.__source_status__:
                        ...
                        self.__chapter__.append(pl.NoEscape(latex.get_person_name_with_reference(v_person.__given_names__, v_person.__surname__, v_person.__gramps_id__) + r' & ' + v_new_binary_string + r' & ' + v_source_status['b'] + r' & ' + v_source_status['m'] + r' & ' + v_source_status['d'] + r'\\'))
                    else:
                        self.__chapter__.append(pl.NoEscape(latex.get_person_name_with_reference(v_person.__given_names__, v_person.__surname__, v_person.__gramps_id__) + r' & ' + v_new_binary_string + r'\\'))

                    v_next_generation_list.append((v_person.get_father_handle(), v_new_binary_string))
                    v_next_generation_list.append((v_person.get_mother_handle(), v_new_binary_string))
    ...
    def __create_generation_list__(self, p_person_handle: str, p_generation_index):
        if p_person_handle is not None:
            ...
            v_sibling_handle_list = v_person.get_sibling_handles()
            ...
            # Process the parents
            self.__create_generation_list__(v_person.get_father_handle(), p_generation_index + 1)
            self.__create_generation_list__(v_person.get_mother_handle(), p_generation_index + 1)
    ...
    def create_ahnentafel_chapter(self):
        ...
        v_subject = person.Person(self.__person_handle__, self.__cursor__)

        # Display progress
        logging.info("Writing the Ahnentafel...")

        # Create a new chapter for the active person
        self.__chapter__ = latex.Chapter(title=language.translate('pedigree of') + ' ' + v_subject.__given_names__ + ' ' + v_subject.__surname__, label=False)
